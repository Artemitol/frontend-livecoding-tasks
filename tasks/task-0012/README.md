# Динамическая коллекция валидных полей
...
Ручная проверка: сначала проверьте начальные `vue` и `angular`, затем очистите список и убедитесь, что сохранение недоступно; добавьте три поля, по очереди введите в них `react` и проверьте строку `Сохранено: react, react, react`.
