...
Ручная проверка: по очереди откройте и закройте профиль и справку; убедитесь, что второй триггер остаётся доступен, чужое окно не появляется, а статус подтверждает callback закрытого экземпляра.
