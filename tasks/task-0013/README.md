...
Ручная проверка: раскройте `src`, затем `utils`, выберите `format.ts` и проверьте `✓ 🟦`; закройте и снова откройте `utils`, выберите `README.md`, затем убедитесь, что `output` на каждом шаге называет правильный узел и действие.
