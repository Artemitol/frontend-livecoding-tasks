...
while IFS= read -r taskFile; do
  ...
  test "$(rg -n '^<details>$' "$taskFile" | wc -l | tr -d ' ')" -eq 5; test "$(rg -n '^<summary>Решение</summary>$' "$taskFile" | wc -l | tr -d ' ')" -eq 1; test "$(rg -n '^<summary>О задаче</summary>$' "$taskFile" | wc -l | tr -d ' ')" -eq 1; ! rg -n '^<details[^>]*\bopen\b' "$taskFile"
...
for number in $(seq 1 30); do
  ...
  mapfile -t simulationTasks < <(rg -o 'tasks/task-[0-9]{4}/README\.md' "$file" | sort -u); test "${#simulationTasks[@]}" -ge 2; test "$(rg -o '\]\([^)]*tasks/task-[0-9]{4}/README\.md\)' "$file" | sort -u | wc -l | tr -d ' ')" -eq "${#simulationTasks[@]}"
  total=0; collections=''
  for taskFile in "${simulationTasks[@]}"; do minutes="$(rg -o -- '- Примерное время: [0-9]+' "$taskFile" | sed -E 's/.*: //')"; total=$((total + minutes)); rg -q "$taskFile.*$minutes минут" "$file"; thematicFile="$(rg -l "\]\([^)]*$taskFile\)" collections | rg -v '^collections/(interviews|real-work)/' | head -n 1)"; collections="$collections$thematicFile\n"; done
...
while IFS= read -r file; do while IFS= read -r link; do target="${link%%#*}"; test -z "$target" || test -e "$(dirname "$file")/$target"; done < <(rg -o '\]\((\.\.?/|[^:)]+)[^)#]*(#[^)]*)?\)' "$file" | sed -E 's/.*\]\(([^)]+).*/\1/'); done < <(find README.md tasks collections -type f -name README.md)
