...
- связывает real-work membership только с полем `Формат`: карточка любого
  технологического профиля с `Приближённая к реальной работе` появляется ровно
  один раз в `collections/real-work/README.md`, а focused-карточка не появляется
  там ни разу;
