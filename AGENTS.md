...
- Keep starter code in one complete minimal copy-ready block by default. Several files are permitted only for a real-work mini-project with genuine file boundaries; every starter and solution block then begins with a language-appropriate `FILE: <path>` comment, contains the complete file without omissions, and uses the same file split.
...
- `Решение` contains only changed code or changed real files, a short explanation, the expected result, and a simple manual check. It does not repeat unchanged fixtures, the complete starter, an HTML document, a harness, or unchanged files.
