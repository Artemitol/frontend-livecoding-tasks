...
- Keep the root `README.md` as the student hub with exactly the four `##` blocks `Как пользоваться базой`, `Подборки по направлениям`, `Симуляции собеседований`, and `Где писать код`. It links all ten thematic and three interview collections plus the four approved editors; task tables, metadata-source explanations, agent-maintenance prose, Beads, GRACE, and knowledge-graph terminology do not belong there.
- Keep exactly ten thematic collection pages and three interview pages under `collections/`. Every task appears in exactly one thematic collection and no more than one interview; thematic entries include a linked student title, one plain-language sentence, and exact time, while interview entries include only the linked student title and exact time after the shared timer paragraph.
...
- Use only the technology values `JavaScript`, `TypeScript`, `HTML/CSS`, `HTML/JavaScript`, `HTML/CSS/JavaScript`, and `React/TypeScript`; the format values `Написать код`, `Исправить код`, `Разобрать код`, and `Предсказать результат`; and the difficulty values `Базовая`, `Средняя`, and `Продвинутая`.
- Use the ordinary code-first card by default in this exact order: title, `← Все подборки`, target-editor instruction, `Условие`, one complete starter block with the assignment in code comments, `Готово, когда`, hint instruction, three closed hints, closed `Решение`, closed `Самопроверка`, optional closed `Почитать по теме`, closed `О задаче`, browser-back instruction and fallback link.
- Use the complex card only when a code comment cannot carry enough context. Its opening order is title, `← Все подборки`, short visible `Условие`, target-editor instruction, `Код — вставьте его в редактор`, then one or more complete starter blocks; the remaining order matches the ordinary card.
- Keep all starter code in one complete copy-ready block unless several real files are necessary. If several starter blocks exist, every starter and solution block begins with a language-appropriate `FILE: <path>` comment, contains the complete file without omissions, and the starter and solution use the same file split.
...
- Include exactly three distinct closed hints with the summaries `Подсказка 1 — куда смотреть`, `Подсказка 2 — с чего начать`, and `Подсказка 3 — почти решение`. Keep `Решение`, `Самопроверка`, `О задаче`, and optional `Почитать по теме` in independent `<details>` blocks without the `open` attribute.
...
- Before a migrated card can pass, record `CardMigrationEvidence` with slug, mode, editor profile, source learning goal, source prerequisite, source runtime assumption, destination locations, target-editor expected result, actual result, and verdict. A temporary harness is diagnostic only and never substitutes for final evidence in the named target editor.
