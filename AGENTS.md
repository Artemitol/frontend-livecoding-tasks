...
- A task belongs to exactly one thematic collection. Its row contains the exact linked card title, one nonempty plain-language sentence, and the exact `N минут` value from card metadata. A React real-work card also belongs to `collections/real-work/README.md`. The root keeps its four current second-level sections and links one interview index, not individual simulations.
