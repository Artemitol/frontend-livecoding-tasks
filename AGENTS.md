...
- `О задаче` contains exactly `Технология`, `Подборка`, `Формат`, `Сложность`, and `Примерное время`. Keep source repositories, authors, migration/import language, and provenance links out of every student-facing card, collection, root page, and simulation.
- A task belongs to exactly one thematic collection; a React real-work card also belongs to `collections/real-work/README.md`. The root keeps its four current second-level sections and links one interview index, not individual simulations.
